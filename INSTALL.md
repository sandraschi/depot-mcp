...
4. Start the server:
   ...
   uv run python -m depot_mcp.server

   # HTTP mode (for web dashboard)
   uv run uvicorn depot_mcp.server:app --port 10727
   ```
5. Open `http://localhost:10727` or the frontend URL.
...
| Port conflict | Run `just fleet-stop` (or `just zombies`) to clear fleet ports |
| Dependencies out of sync | `uv sync --all-extras` |
| Something else | [Open a GitHub issue](https://github.com/sandraschi/depot-mcp/issues) |
